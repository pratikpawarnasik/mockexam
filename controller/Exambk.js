...
// timepicker for select exam end time
	...
	/*
	$scope.openAddPara = function(){
		if(!$scope.examDet.course > 0){
			alert("Please select course.");
			return false;
		}
		$scope.param = {
			'courseid' : $scope.examDet.course
		}
		var my_url = url_getcoursechaptetopic+$.param($scope.param);
		$rootScope.loading = true;
		$http({
              method : 'GET',
              url : my_url,
              headers : {'authcode': $scope.authcode}
            	...
            	{
            		$scope.courseHArr = response.courseHirarchi;
				}
				else{
					$scope.courseHArr = [];
					//alert("Course are either deleted or not inserted.");
				}
		...
		$scope.searchchapter = '';
		$scope.paraArr = [];           
		$("#showParaModal").modal('show');
	}
	*/
	//close para popup
	/*$scope.closePara = function(){
		$("#showParaModal").modal('hide');
	}*/
	
	//get paragraph data with chapter/topic
/*	$scope.loadParaData = function(id){	
		for(var i=0;i<$scope.courseHArr.length;i++){
			if($scope.courseHArr[i]['id'] == id){
				$scope.searchtype = $scope.courseHArr[i]['type'];
			}
		}			
		$scope.param = {
			//'instid' : $scope.instid,
			'userid' : $scope.userid,
			'chapterid' : $scope.searchchapter,
			'searchtype' : $scope.searchtype
		}
		$scope.paraMsg = '';
		var my_url = url_getchaptetopicpara+$.param($scope.param);
		$rootScope.loading = true;
		$http({
		   method : 'GET',
		   url : my_url,
		   headers : {'authcode': $scope.authcode}
				...
				$rootScope.loading = false;
		 	if(response.status == 200){
		 		$scope.paraArr = response.paragraph;
			}
			else{
				$scope.paraArr = [];
				$scope.paraMsg = "Paragraph are either deleted or not inserted.";
			}
		}).error(function(error){
				$rootScope.loading = false;
				$scope.paraMsg = "Some unknown error has occurred. Please try again.";
		});
